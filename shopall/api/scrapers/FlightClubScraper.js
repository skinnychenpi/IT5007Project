...
var linkAndLowestResellPriceAndDescription = {platform: "FlightClub"};

async function getLink(styleID) {

    const response = await got.post('https://2fwotdvm2o-dsn.algolia.net/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20vanilla%20JavaScript%20(lite)%203.32.0%3Breact-instantsearch%205.4.0%3BJS%20Helper%202.26.1&x-algolia-application-id=2FWOTDVM2O&x-algolia-api-key=ac96de6fef0e02bb95d433d8d5c7038a', {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 6.2; rv:20.0) Gecko/20121202 Firefox/20.0',
                'Content-Type': 'application/json'

            },
            body:  "{\"requests\":[{\"indexName\":\"product_variants_v2_flight_club\",\"params\":\"query=" + styleID + "&hitsPerPage=1&maxValuesPerFacet=1&filters=&facets=%5B%22lowest_price_cents_usd%22%5D&tagFilters=\"}]}"
        });
    ...
    if (json.results[0].hits[0]) {
        if (json.results[0].hits[0].lowest_price_cents_usd / 100 != 0) {
            linkAndLowestResellPriceAndDescription["lowestResellPrice"] = json.results[0].hits[0].lowest_price_cents_usd / 100;
        }
        linkAndLowestResellPriceAndDescription["resellLinks"] = 'https://www.flightclub.com/' + json.results[0].hits[0].slug;
        linkAndLowestResellPriceAndDescription["description"] = json.results[0].hits[0].story;
    } else {
        linkAndLowestResellPriceAndDescription["lowestResellPrice"] = -1;
        linkAndLowestResellPriceAndDescription["resellLinks"] = "";
        linkAndLowestResellPriceAndDescription["description"] = "";
    }

    return linkAndLowestResellPriceAndDescription;
...
async function getPrices(resellLinks, styleID) {
    if (!resellLinks) {
        console.log("Can not find resell links at FlightClub.");
    } else {
        let slug = resellLinks.split('.com/')[1];
        try {
            ...
            try {
                ...
                let err = new Error("Could not connect to Flight Club while searching '" + styleID + "' Error: ", error)
                console.log(err);
                }
        } catch (error) {
...
async function workFlow() {
    var dict = await getLink("GW3773");
    console.log(dict);

    var priceMap = await getPrices(dict["resellLinks"],"GW3773");
